...
Clone/fork project and adjust `config.py`. The minimum change is the path to your markdown files directory.
...
Install the Python requirements (preferably in a virtualenv):
...
This avoids common macOS conflicts on port 5000.
...
The app prints the exact URL on startup, for example:

    Open the app at: http://127.0.0.1:5050

Open <http://127.0.0.1:5050> (or <http://localhost:5050>) in your browser.
